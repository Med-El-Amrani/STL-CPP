...
#include <signal.h>
#include <sys/time.h>
#include "stl_custom/containers/dynamic_vector.hpp" // this project's vector (compile with -Iinclude)

// The tests below are written against the historical `ft::` namespace.
// Map it onto this project's `stl_custom` namespace; reverse_iterator is not
// reimplemented here, so it comes from the STL.
namespace ft {
template <typename T>
using vector = stl_custom::dynamic_vector<T>;
